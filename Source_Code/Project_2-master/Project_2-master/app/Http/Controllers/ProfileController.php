...
class ProfileController extends Controller
{
    public function request($id)
    {
        ...
        $papers = Paper::with('teacher', 'author', 'sources')->whereHas('teacher', function ($query) use ($id) {
            $query->where('users.id', '=', $id);
        })->orderBy('paper_yearpub', 'desc')->get();

        $papers_scopus = Paper::with('teacher', 'author', 'sources')->whereHas('teacher', function ($query) use ($id) {
            $query->where('users.id', '=', $id);
        })->orderBy('paper_yearpub', 'desc')->whereHas('sources', function ($query) {
            $query->where('source_data_id', '=', 1);
        })->get();

        $papers_wos = Paper::with('teacher', 'author', 'sources')->whereHas('teacher', function ($query) use ($id) {
            $query->where('users.id', '=', $id);
        })->whereHas('sources', function ($query) {
            $query->where('source_data_id', '=', 2);
        })->orderBy('paper_yearpub', 'desc')->get();

        $papers_tci = Paper::with('teacher', 'author', 'sources')->whereHas('teacher', function ($query) use ($id) {
            $query->where('users.id', '=', $id);
        })->whereHas('sources', function ($query) {
        ...
        foreach ($year as $key => $value) {
            $paper_scopus[] = Paper::with('teacher')->whereHas('sources', function ($query) {
        ...
        foreach ($year2 as $key => $value) {
            $paper_scopus_s[] = Paper::with('teacher')->whereHas('sources', function ($query) {
                return $query->where('source_data_id', '=', 1);
            })->whereHas('teacher', function ($query) use ($id) {
                $query->where('users.id', '=', $id);
            })->where(DB::raw('(paper_yearpub)'), $value)->count();
        ...
        foreach ($year2 as $key => $value) {
            $paper_tci_s[] = Paper::whereHas('sources', function ($query) {
                return $query->where('source_data_id', '=', 3);
            })->whereHas('teacher', function ($query) use ($id) {
                $query->where('users.id', '=', $id);
            })->where(DB::raw('(paper_yearpub)'), $value)->count();
        ...
        foreach ($year2 as $key => $value) {
            $paper_wos_s[] = Paper::whereHas('sources', function ($query) {
                return $query->where('source_data_id', '=', 2);
            })->whereHas('teacher', function ($query) use ($id) {
                $query->where('users.id', '=', $id);
            })->where(DB::raw('(paper_yearpub)'), $value)->count();
